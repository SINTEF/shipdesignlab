...
class Weather:
    ...
    def get_weather_at_index(self, index):
        ...
        if self.ocean_current_speed_m_per_s is not None:
            weather_index.ocean_current_speed_m_per_s = float(self.ocean_current_speed_m_per_s[index])
        if self.ocean_current_direction_deg is not None:
            weather_index.ocean_current_direction_deg = float(self.ocean_current_direction_deg[index])
        ...
        if self.sea_water_temperature_deg_c is not None:
            weather_index.sea_water_temperature_deg_c = float(self.sea_water_temperature_deg_c[index])
    ...
    def to_proto(self) -> proto.Weather:
        ...
        return proto.Weather(
            significant_wave_height_m=self.significant_wave_height_m[0] if self.significant_wave_height_m is not None else None,
            mean_wave_period_s=self.mean_wave_period_s[0] if self.mean_wave_period_s is not None else None,
            wave_direction_deg=self.wave_direction_deg[0] if self.wave_direction_deg is not None else None,
            wind_speed_m_per_s=self.wind_speed_m_per_s[0] if self.wind_speed_m_per_s is not None else None,
            wind_direction_deg=self.wind_direction_deg[0] if self.wind_direction_deg is not None else None,
            ocean_current_speed_m_per_s=self.ocean_current_speed_m_per_s[0] if self.ocean_current_speed_m_per_s is not None else None,
            ocean_current_direction_deg=self.ocean_current_direction_deg[0] if self.ocean_current_direction_deg is not None else None,
            air_temperature_deg_c=self.air_temperature_deg_c[0] if self.air_temperature_deg_c is not None else None,
            sea_water_temperature_deg_c=self.sea_water_temperature_deg_c[0] if self.sea_water_temperature_deg_c is not None else None,
        )
...
class OperationPoint:
        ...
        if proto_object is not None:
            ...
            for descriptor, value in proto_object.ListFields():
                if descriptor.name == 'weather':
                    ...
                    value = np.array([value]) if isinstance(value, (float, int)) else value
                    setattr(self, descriptor.name, value)
        else:
            self._proto = proto.OperationPoint()
            self.timestamp_seconds = np.array([timestamp_seconds]) \
                if isinstance(timestamp_seconds, (float, int)) else timestamp_seconds
            self.speed_kn = np.array([speed_kn]) \
                if isinstance(speed_kn, (float, int)) else speed_kn
            self.power_limit_kw = np.array([power_limit_kw]) \
                if isinstance(power_limit_kw, (float, int)) else power_limit_kw
            self.heading_deg = np.array([heading_deg]) \
                if isinstance(heading_deg, (float, int)) else heading_deg
            self.auxiliary_power = np.array([auxiliary_power]) \
                if isinstance(auxiliary_power, (float, int)) else auxiliary_power
    ...
    def to_proto(self) -> proto.OperationPoint:
        ...
        return proto.OperationPoint(
            timestamp_seconds=self.timestamp_seconds[0] if self.timestamp_seconds is not None else None,
            ...
            auxiliary_power=self.auxiliary_power[0] if self.auxiliary_power is not None else None,
