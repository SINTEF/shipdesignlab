...
from ship_model_lib.utility import m_per_s_to_kn, get_speed_kn_from_froude_number, kn_to_m_per_s
...
def dtc_propeller_curves() -> PropulsorDataOpenWater:
    data = [
        ...
        for j, kt, kq in zip(
            ...
            np.array([0.509, 0.492, 0.472, 0.450, 0.427, 0.403, 0.378, 0.353, 0.327, 0.302, 0.276, 0.250, 0.225, 0.199, 0.172, 0.145, 0.118, 0.089, 0.058, 0.026, 0.0]),
            np.array([0.713, 0.691, 0.667, 0.640, 0.613, 0.584, 0.554, 0.524, 0.493, 0.462, 0.430, 0.398, 0.366, 0.333, 0.299, 0.264, 0.228, 0.191, 0.151, 0.109, 0.065])
        )
    ]
...
def test_snnm_method_for_resistance_component(
    ...
    wave_incident_angle_list_rad = np.array([0, np.pi / 6, np.pi / 3, np.pi / 2, 2 * np.pi / 3, np.pi * 5 / 6, np.pi])
    ...
    for wave_incident_angle_rad in wave_incident_angle_list_rad:
        ...
        wave_resistance_motion = added_resistance._get_non_dimensional_wave_resistance_due_to_motion(
            wave_frequency_rad_per_s= omega,
            wave_incident_angle_rad=wave_incident_angle_rad,
            vessel_speed_kn=vessel_speed_kn
        )
        wave_resistance_reflection, _ = added_resistance._get_non_dimensional_wave_resistance_due_to_reflection(
            wave_frequency_rad_per_s=omega,
            wave_incident_angle_rad=wave_incident_angle_rad,
            vessel_speed_kn=vessel_speed_kn
        )
        index = 0
        for wave_resistance_component in [wave_resistance, wave_resistance_motion, wave_resistance_reflection]:
            index += 1
            fig.add_scatter(
...
def test_snnm_method_for_wave_resistance(s_cb84_ship_dimension):
    ...
    speed_kn = get_speed_kn_from_froude_number(
        froude_number=froude_number, lpp_m=s_cb84_ship_dimension.lpp_length_between_perpendiculars_m
    )
    ...
    for wave_height, wave_period in zip(wave_height_list, wave_period_list):
        ...
        resistance_stawave2_newton = added_resistance_stawave2.get_added_resistance_newton(
            vessel_speed_kn=speed_kn,
            weather=Weather(significant_wave_height_m=wave_height, mean_wave_period_s=wave_period, wave_direction_deg=0),
            heading_deg=0
        )[0]
        ...
        resistance_newton = np.array([
            added_resistance.get_added_resistance_newton(
                vessel_speed_kn=speed_kn,
                weather=Weather(significant_wave_height_m=wave_height, mean_wave_period_s=wave_period, wave_direction_deg=wave_direction_deg),
                heading_deg=0
            )[0] for wave_direction_deg in wave_heading
        ])
...
def test_snnm_method_for_integration_interval(s_cb84_ship_dimension):
    ...
    for wave_height, wave_period in zip(wave_height_list, wave_period_list):
        ...
        for wave_direction_deg in tqdm(wave_heading):
            start_time = time.time()
            result.append(added_resistance.get_added_resistance_newton(
                vessel_speed_kn=speed_kn,
                weather=Weather(significant_wave_height_m=wave_height,
...
def test_added_resistance_wind():
    ...
    for index in range(number_of_samples):
        fig.add_scatter(x=[0, vector_vs[index][0]], y=[0, vector_vs[index][1]], name=f"Vessel-{index}")
    ...
    rel_wind_speed, rel_wind_speed_angle = added_resistance._get_relative_wind_angle_and_speed(
    ...
    for index in range(number_of_samples):
        ...
        assert rel_wind_speed_angle_ref[index] \
               == pytest.approx(rel_wind_speed_angle[index] * 180 / np.pi)
...
def test_added_resistance_wave_with_ntnu_head_sea(added_resistance_ntnu_general_cargo):
    ...
    path_to_ref_data = os.path.join(os.path.dirname(__file__), "Liu_method_alldir_output_head_sea.csv")
    ...
    theta = 0
    a1 = added_resistance_ntnu_general_cargo._get_a1(
        wave_frequency_rad_per_s=omega,
        wave_incident_angle_rad=np.deg2rad(theta),
        vessel_speed_kn=vs_kn,
    ) * 60.3 * cb ** 1.34 * (4 * kyy) ** 2
    ...
    d1[w_normalized >= 1.0] = -566 * np.power(lpp / b, -2.66) \
                                  * (4 - 125 * added_resistance_ntnu_general_cargo._trim_angle_rad)
    r_awr, r_awr_comp = added_resistance_ntnu_general_cargo._get_non_dimensional_wave_resistance_due_to_reflection(
        wave_frequency_rad_per_s=omega,
        wave_incident_angle_rad=np.deg2rad(theta),
        vessel_speed_kn=vs_kn,
    )
...
def test_added_resistance_wave_with_ntnu_following_sea_120(added_resistance_ntnu_general_cargo):
    """Test the added resistance by wave with NTNU data"""
    show_plot = False
...
def test_added_resistance_ntnu_general_cargo_arbitrary_heading(added_resistance_ntnu_general_cargo):
    """Test added resistance for general cargo ship."""
    show_plot = False
    path_to_ref_data = os.path.join(os.path.dirname(__file__), "Liu_method_alldir_output_arbitrary_wave_dir.csv")
    ref_data = pd.read_csv(path_to_ref_data, index_col=0)
    lpp = added_resistance_ntnu_general_cargo.ship_dimension.lpp_length_between_perpendiculars_m
    wave_length_normalized = np.linspace(0.15, 2.04, 190)
    wave_length_m = wave_length_normalized * lpp
