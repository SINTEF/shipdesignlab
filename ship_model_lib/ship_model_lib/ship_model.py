...
# %% auto 0
__all__ = ['Numeric', 'CalmWaterModel', 'PropulsorModel', 'AddedResistanceWaveModel', 'PowerSystem', 'ShipDescription',
           'HullOperatingPoint', 'HullData', 'PropulsorData', 'ShipPerformanceData', 'ShipModel']
...
class HullOperatingPoint:
    ...
    def total_resistance_newton(self):
        return self.calm_water_resistance_newton \
...
class ShipModel:
    def __init__(
        ...
        if not ship_description:
            self.ship_description=ShipDescription(name="ship_name_unknown",
                                                  type=ShipType("ship_type_unknown"))
        else:
    ...
    def _get_calm_water_power_speed_curve_interpolator(self):
        ...
        return interpolate.interp1d(np.array(power_list_kw), np.array(speed_list_kn), kind="linear", fill_value="extrapolate")
    ...
    def get_ship_performance_data_from_speed(
        ...
        if not isinstance(self.calm_water_resistance, CalmWaterResistanceBySpeedPowerCurve) \
            ...
            hull_operating_point.calm_water_resistance_newton = calm_water_resistance_kilo_newton * 1000
            ...
            if self.added_resistance_wave is not None and weather is not None:
                ...
                if not added_resistance_by_sea_margin:
                    ...
                    added_resistance_wave_newton = calm_water_resistance_kilo_newton \
                                                   * sea_margin_percent / 100 * 1000
            else:
                added_resistance_wave_newton = 0
            if self.added_resistance_wind is not None \
        ...
        # For power curve models, we need to calculate the shaft power directly
            ...
            shaft_power_kw = self.calm_water_resistance.get_power_from_speed(
                speed_kn=vessel_speed_kn).value
            if np.isscalar(vessel_speed_kn):
                n_rps = 1 if vessel_speed_kn > 0 else 0
                torque_newton_meter = shaft_power_kw / rps_to_rad_per_s(n_rps) \
                    if shaft_power_kw > 0 else 0
            else:
                ...
                torque_newton_meter[shaft_power_kw > 0] = \
                    shaft_power_kw[shaft_power_kw > 0] / rps_to_rad_per_s(n_rps[shaft_power_kw > 0])
    ...
    def get_ship_performance_data_from_power(
        ...
        return self.get_ship_performance_data_from_speed(
            vessel_speed_kn=vessel_speed_kn, weather=weather, auxiliary_power_kw=auxiliary_power_kw
        )

    def get_ship_performance_data_from_operating_point(self, operation_point: OperationPoint) -> ShipPerformanceData:
        """Calculates the ship performance data from the operating point.

        :param operation_point: The operating point
        :return: The ship performance data
        """
        performance_data_from_speed = self.get_ship_performance_data_from_speed(vessel_speed_kn=operation_point.speed_kn,
        ...
        if self.machinery_system is not None:
            if self.machinery_system.propulsion_type is PropulsionType.MECHANICAL:
                power_on_source_kw = performance_data_from_speed.power_source_data.mechanical_system.power_on_source_kw
            elif self.machinery_system.propulsion_type is PropulsionType.ELECTRIC:
                power_on_source_kw = performance_data_from_speed.power_source_data.electric_system.power_on_source_kw
            else:
                raise NotImplementedError("The power source data is not implemented for the propulsion type: {self.machinery_system.propulsion_type}")
        elif self.machinery_system is None and self.propulsor is not None:
            power_on_source_kw = performance_data_from_speed.propeller_data.shaft_power_kw
        elif self.machinery_system is None and self.propulsor is None:
            power_on_source_kw = performance_data_from_speed.hull_data.total_towing_power_kw
        else:
            power_on_source_kw = 0

        if power_on_source_kw > operation_point.power_limit_kw:
            performance_data_from_power = self.get_ship_performance_data_from_power(power_out_source_kw=operation_point.power_limit_kw,
                                                                                    weather=operation_point.weather,
                                                                                    heading_deg=operation_point.heading_deg,
                                                                                    auxiliary_power_kw=operation_point.auxiliary_power)
