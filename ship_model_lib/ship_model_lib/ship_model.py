        ...
        if np.any(greater_than_power_limit):
            for index, greater_than_power_limit_each in enumerate(
                ...
                if greater_than_power_limit_each:
                    if operation_point.weather is not None:
                        if (
                            operation_point.weather.significant_wave_height_m
                            is not None
                        ):
                        ...
                        if operation_point.weather.mean_wave_period_s is not None:
                            mean_wave_period_s = (
                                operation_point.weather.mean_wave_period_s[index]
                            )
                        else:
                            mean_wave_period_s = (None,)
                        if operation_point.weather.wave_direction_deg is not None:
                            wave_direction_deg = (
                                operation_point.weather.wave_direction_deg[index]
                            )
                        else:
                            wave_direction_deg = None
                        if operation_point.weather.wind_speed_m_per_s is not None:
                            wind_speed_m_per_s = (
                                operation_point.weather.wind_speed_m_per_s[index]
                            )
                        else:
                            wind_speed_m_per_s = None
                        if operation_point.weather.wind_direction_deg is not None:
                            wind_direction_deg = (
                                operation_point.weather.wind_direction_deg[index]
                            )
                        else:
                            wind_direction_deg = None
                        if (
                            operation_point.weather.ocean_current_speed_m_per_s
                            is not None
                        ):
                        ...
                        if (
                            operation_point.weather.ocean_current_direction_deg
                            is not None
                        ):
                        ...
                        weather = None
                    if operation_point.power_limit_kw is not None:
                        try:
                            power_limit_kw = operation_point.power_limit_kw[index]
                        except IndexError:
                            power_limit_kw = operation_point.power_limit_kw
                    else:
                        power_limit_kw = 1e6
