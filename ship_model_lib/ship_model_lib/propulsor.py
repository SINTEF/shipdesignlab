...
# %% auto 0
__all__ = ['Numeric', 'PropulsorOperatingPoint', 'WakeFractionThrustDeductionFactorPoint', 'OpenWaterPropellerCurvePoint',
           'ReCorrection', 'Propulsor', 'kn_to_m_per_s', 'm_per_s_to_kn', 'rps_to_rad_per_s', 'PropulsorDataOpenWater',
           'PropulsorDataBseries', 'PropulsorDataScalar']
...
class PropulsorDataOpenWater(Propulsor):
    ...
    def __init__(
        ...
        if wake_thrust_reduction and len(wake_thrust_reduction) == 1:
            ...
            thrust_red_array = np.array([pt.thrust_deduction_factor for pt in wake_thrust_reduction])
            self._interpolate_wake_factor = interpolate.PchipInterpolator(
    ...
    def plot_open_water_curves(self):
        ...
        plt.plot(self._j, self._kt, "o", self._j, 10 * self._kq, "x", self._j, efficiency_ref, "*")  # plot openwater data
        ...
        plt.show()
        print('\n Maximum efficiency is {:.2f} at J = {:.2f}'
              .format(max(efficiency), j[np.argmax(efficiency)]))
    ...
    def _get_thrust_reduction(self, vessel_speed_kn: float) -> float:
        return 0.0 if self._interpolate_thrust_reduction is None \
    ...
    def _function_to_solve_for_n_rps(
        ...
        return self.get_thrust_reduced(n_rps=n_rps, vessel_speed_kn=vessel_speed_kn) \
    ...
    def get_propulsor_data_from_vessel_speed_thrust(
        self,
        vessel_speed_kn: Numeric,
        thrust_resistance_newton: Numeric
    ) -> PropulsorOperatingPoint:
        ...
        for speed, thrust in zip(vessel_speed_kn, thrust_resistance_newton):
            ...
            sol = root_scalar(
                ...
                x0=initial_n_rps, x1=initial_n_rps*1.1,
                args=(speed, thrust)
            )
            if sol.converged:
...
class PropulsorDataBseries(PropulsorDataOpenWater):
    ...
    def __init__(
        ...
        self._multKT = (self._coefficients[0] * (self._pod ** self._coefficients[1]) *
                        (self._ear ** self._coefficients[2]) * (self._z ** self._coefficients[3]))
        self._multKQ = (self._coefficients[4] * (self._pod ** self._coefficients[5]) *
                        (self._ear ** self._coefficients[6]) * (self._z ** self._coefficients[7]))
    ...
    def _re_correction_ittc78(self, KT, KQ):
        ...
        c = 2.168 / (self._z / self._ear) * self._d if self._z == 3 else 2.144 / (self._z / self._ear) * self._d
    ...
    def _re_correction_bseries(self, KT, KQ, j):
        if self._re > 2e6:  # apply Reynolds number correction id Re is greater than 2e6
            ...
            KT += (self._dKT[0] + self._dKT[1] * self._ear * j ** 2 + self._dKT[2] * self._ear * self._pod * j +
                   ...
                   self._dKT[8] * (np.log(self._re) - 0.301) * self._z ** 2 * self._ear * self._pod ** 3 * j)

            KQ += (self._dKQ[0] + self._dKQ[1] * self._pod + self._dKQ[2] * self._z * self._pod ** 6 + self._dKQ[3] * self._ear ** 2 +
                   ...
                   self._dKQ[8] * (np.log(self._re) - 0.301) ** 2 * self._z * self._ear * self._pod * j +
...
class PropulsorDataScalar(Propulsor):
    ...
    def get_propulsor_data_from_vessel_speed_thrust(
        ...
        if np.isscalar(n_rps):
            torque_newton_meter = shaft_power_kw /  (n_rps * 2 * np.pi) * 1000 if n_rps > 0.0 else 0.0
        else:
        ...
        return PropulsorOperatingPoint(vessel_speed_kn=vessel_speed_kn,
                                       n_rpm=n_rps * 60,
                                       j=0,
