...
class PropulsorOperatingPoint:
    ...
    def efficiency_hull(self) -> Numeric:
        ...
        try:
            hull_efficiency = (1 - self.thrust_deduction_factor) / (1 - self.wake_fraction_factor)
...
class PropulsorDataOpenWater(Propulsor):
    ...
    def __init__(
        ...
        if wake_thrust_reduction and len(wake_thrust_reduction) == 1:
            ...
            wake_array = np.array([pt.wake_fraction_factor for pt in wake_thrust_reduction])
    ...
    def plot_open_water_curves(self):
        ...
        plt.plot(j, self.get_kt(j), j, 10 * self.get_kq(j), j, efficiency)  # plot interpolated data
        plt.xlabel("Advance Ratio J")
        plt.ylabel("KT, 10*KQ, Efficiency")
        plt.title("Open water Curves")
        plt.legend(["KT", "KQ", r"\eta"])
        plt.show()
        print(
            f"\n Maximum efficiency is {max(efficiency):.2f} at J = {j[np.argmax(efficiency)]:.2f}"
        )
    ...
    def _get_wake_velocity_m_per_s(self, vessel_speed_kn: float) -> float:
        ...
        return kn_to_m_per_s(vessel_speed_kn * (1 - self._interpolate_wake_factor(vessel_speed_kn)))
    ...
    def get_propulsor_data_from_vessel_speed_rps(
        ...
        if len(vessel_speed_kn) != len(n_rps):
            raise ValueError("vessel_speed_kn and n_rps must be of same length or scalar")
        ...
        efficiency_open_water[valid_indices] = self.get_efficiency_open_water(j[valid_indices])
        return PropulsorOperatingPoint(
            vessel_speed_kn=vessel_speed_kn,
            n_rpm=n_rps * 60,
            j=j,
            wake_velocity_kn=m_per_s_to_kn(self._get_wake_velocity_m_per_s(vessel_speed_kn)),
...
class PropulsorDataBseries(PropulsorDataOpenWater):
    ...
    def _generate_kt_kq_curve(self, j_step: float = 0.01) -> list[OpenWaterPropellerCurvePoint]:
    ...
    def _get_kt_bseries(self, j_value: Numeric) -> Numeric:
        ...
        kt = np.array([(sum((j ** self._coefficients[8]) * self._multKT)) for j in j_value])
        return kt

    def _get_kq_bseries(self, j_value: Numeric) -> Numeric:
        if isinstance(j_value, float):
            j_value = np.array([j_value])
        kq = np.array([sum((j ** self._coefficients[9]) * self._multKQ) for j in j_value])
        return kq
    ...
    def _re_correction_ittc78(self, KT, KQ):
        ...
        CD_model = 2 * (1 + 2 * t / c) * (0.044 / self._re ** (1 / 6) - 5 / self._re ** (2 / 3))
    ...
    def _re_correction_bseries(self, KT, KQ, j):
        if self._re > 2e6:  # apply Reynolds number correction id Re is greater than 2e6
            KT += (
                ...
                + self._dKT[6] * (np.log(self._re) - 0.301) ** 2 * self._z * self._ear * j**2
                + self._dKT[7] * (np.log(self._re) - 0.301) * self._z * self._ear * self._pod * j
                + self._dKT[8]
            ...
            KQ += (
                ...
                * j
                ...
                + self._dKQ[10] * (np.log(self._re) - 0.301) ** 2 * self._z * self._pod**6
...
class PropulsorDataScalar(Propulsor):
    ...
    def get_propulsor_data_from_vessel_speed_thrust(
        ...
        shaft_power_kw = thrust_resistance_newton * vessel_speed_m_per_s / self.efficiency / 1000
